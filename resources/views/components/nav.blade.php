<header class="w-full py-normal max-md:px-4 fixed grid-basic">
    ...
    <div class="flex justify-between items-center relative z-40 col-span-full md:col-start-2 md:col-end-12">
        ...
        <nav aria-labelledby="main-navigation"
            ...
            <h2 class="sr-only" id="main-navigation">{{ __('nav.main_navigation') }}</h2>
            <ul
                class="flex max-lg:flex-col max-lg:gap-huge justify-end gap-8 lg:items-center my-auto lg:text-sm xl:text-base">
                <x-nav-item :label="__('nav.homepage')" :route="route('home.client')"
                            :class="request()->routeIs('home.client') ? 'active' : 'link'"/>
                <x-nav-item :label="__('nav.happypaws')" :route="route('paws.client')"
                            :class="request()->routeIs('paws.client') ? 'active' : 'link'"/>
                <x-nav-item :label="__('nav.animals')" :route="route('animals.client')"
                            :class="request()->routeIs('animals.client') || request()->routeIs('animals.client.show') ? 'active' : 'link'"/>
                <x-nav-item :label="__('nav.contact')" :route="route('contact.client')" :class="''"
                            :aClass="'button-dark'"/>
            </ul>
        </nav>
