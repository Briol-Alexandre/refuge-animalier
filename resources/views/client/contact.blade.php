<x-layouts.client-layout>
    <section aria-labelledby="contact-title" class="grid-basic">
        ...
        <form action="{{route('contact.send')}}" method="post"
            ...
            <x-form.select :label="__('contact.subject')" :id-name="'object'" :class="'col-span-full'">
                <option value="">{{__('contact.subject_placeholder')}}</option>
                <option value="volunteer">{{__('contact.subject_1')}}</option>
                <option value="other">{{__('contact.subject_2')}}</option>
            </x-form.select>
            <x-form.input-textaria :class="'col-span-full'" :input-class="'min-h-32'" :name-id="'message'"
                                   :label="__('contact.message_label')"
