<template>
    <div class="border rounded-lg bg-white p-2 overflow-y-scroll">
        <div class="flex justify-between items-center mb-2">
            <p class="font-atten font-bold text-2xl">Espèces</p>
            <button class="hover:cursor-pointer underline" @click="handleCreateModal">+ Ajouter une
                espèce
            </button>
        </div>
        <ul class="flex flex-col gap-2">
            <li class="p-2 border rounded-lg flex justify-between" v-for="specie in species">
                {{ specie.name }}
                <div class="flex item-center gap-2">
                    <button @click="openModal(specie)" class="hover:cursor-pointer">
                        <Edit />
                    </button>
                    <button class="hover:cursor-pointer" @click="handleDeleteModal(specie)">
                        <Delete />
                    </button>
                </div>
            </li>
    ...
    <Teleport to="body">
        <Modal :condition="isModalOpen" @close="openModal" index="z-30" modal-classes="max-w-[500px]">
            <p class="title">Modifier l'espèce</p>
...
import { destroy, store, update } from '@/actions/App/Http/Controllers/SpeciesController.js';
    ...
    methods: {
        ...
        submit() {
            this.form.patch(update(this.modelToEdit.id), {
                    onSuccess: (page) => {
                        this.toast.success({ text: 'Espèce modifiée avec succès !' });
                        this.form.reset();
        ...
        deleteSpecie() {
            this.form.delete(destroy(this.modelToEdit.id), {
                    onSuccess: (page) => {
                        this.toast.success({ text: 'Espèce supprimée avec succès !' });
                        this.form.reset();
