<template>
    ...
    <div class="border rounded-lg bg-white p-2 overflow-y-scroll">
        <div class="flex justify-between items-center mb-2">
            <p class="font-atten font-bold text-2xl">Races</p>
            <button class="hover:cursor-pointer underline" @click="handleCreateModal">+ Ajouter une
                espèce
            </button>
        </div>
        <ul class="flex flex-col gap-2">
            <li class="p-2 border rounded-lg flex justify-between" v-for="breed in breeds">
                <p>
                    {{ breed.name }}
                ...
                <div class="flex item-center gap-2">
                    <button @click="openModal(breed)" class="hover:cursor-pointer">
                        <Edit/>
                    </button>
                    <button class="hover:cursor-pointer" @click="handleDeleteModal(breed)">
                        <Delete/>
                    </button>
                </div>
            </li>
    ...
    <Teleport to="body">
        <Modal :condition="isModalOpen" @close="openModal" index="z-30" modal-classes="max-w-[500px]">
            <p class="title">Modifier la race</p>
...
import {
    ...
    store,
    update
} from '@/actions/App/Http/Controllers/BreedsController.js';

export default {
    name: "",
    components: {Select, Modal, Input, InputError, Delete, Edit, Button},
    props: ['breeds', 'species'],
